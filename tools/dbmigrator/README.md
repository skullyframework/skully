# Additional info:
Do ./ruckus.php instead of php ruckus.php, otherwise mysqli won't load.

table_prefix won't be applied to:
- execute
- select_all
- select_one
See in lib/Ruckusing/Migration/Base.php for usage.
...
The available methods are (brief list below, with detailed usage further down):
...
```php
    $result = $this->select_all("SELECT email, first_name, last_name FROM users WHERE createdAt >= SUBDATE( NOW(), INTERVAL 7 DAY)");
